...
```
go install github.com/artemreyt/vtprotobuf/cmd/protoc-gen-go-vtproto@main
```
...
To use `vtprotobuf` with the new versions of GRPC, you need to register the codec provided by the `github.com/artemreyt/vtprotobuf/codec/grpc` package.

```go
package servenv

import (
    "github.com/artemreyt/vtprotobuf/codec/grpc"
...
To use `vtprotobuf` as a DRPC encoding, simply pass `github.com/artemreyt/vtprotobuf/codec/drpc` as the `protolib` flag in your `protoc-gen-go-drpc` invocation.

Example:

```
protoc --go_out=. --go-vtproto_out=. --go-drpc_out=. --go-drpc_opt=protolib=github.com/artemreyt/vtprotobuf/codec/drpc
```
